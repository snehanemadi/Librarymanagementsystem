<?php
	...
	$row = mysqli_fetch_assoc($result);*/
		...
		$reg = "UPDATE journals SET status='available' WHERE JNO='$jno'";
		mysqli_query($con, $reg);
		$reg1 = "UPDATE staff SET no_of_books = no_of_books+1 WHERE SUSN='$lsid'";
		mysqli_query($con, $reg1);
	
	$start = .date();
		$end = date_add($start,date_interval_create_from_date_string("15 days"));
		$tid = .rand ();
		$reg3 = " insert into transac(TID, start_date,end_date,T_JNO,T_PNO,T_ISBN) values ('$tid', '$start', '$end', '$jno', ' ', ' ')";
